...
void ObtainResults(std::string* scoreName, int scorePlayer[], int &totalScorePlayer, bool showScore, bool &isThereAStair){
    ...
    }else if (scorePlayer[1] >= scorePlayer[0] && scorePlayer[1] >= scorePlayer[2])
    {
        ...
        std::cout << "El jugador " << *scoreName << " tiene como mayor puntaje " << scorePlayer[1] << " en el lanzamiento " << 2 << " sumandolo a tu total de puntos..." << std::endl;
        std::cout << "El total de puntaje que llevas es: " << totalScorePlayer << std::endl;
    }else if (scorePlayer[2] >= scorePlayer[1] && scorePlayer[2] >= scorePlayer[0])
    {
        totalScorePlayer += scorePlayer[2];
        std::cout << "El jugador " << *scoreName << " tiene como mayor puntaje " << scorePlayer[2] << " en el lanzamiento " << 3 << " sumandolo a tu total de puntos..." << std::endl;
        std::cout << "El total de puntaje que llevas es: " << totalScorePlayer << std::endl;
    }
    if (isThereAStair == true)
    {
        std::cout << "El jugador " << *scoreName << " ha obtenido una escalera! Has ganado en este turno!" << std::endl;
    }
}
void StartGame(std::string* scoreName, int scorePlayer[], int turn, bool &hiddenOptionSelected, bool &diceAnimation, bool &dicePerTime, bool &showScore){
    for (int i = 0; i < 3; i++)
    {
        rlutil::cls();
        if (*scoreName == "")
        {
            *scoreName = NamePlayer();
            scorePlayer[i] = RollDice(scoreName, turn, i + 1, hiddenOptionSelected, diceAnimation, dicePerTime);
        }else {
            scorePlayer[i] = RollDice(scoreName, turn, i + 1, hiddenOptionSelected, diceAnimation, dicePerTime);
        }
        if (scorePlayer[i]==0 || scorePlayer[i]==-1)
        {
            break;
        }else {
            std::cout << "El jugador " << *scoreName << " hizo " << scorePlayer[i] << " puntos." << std::endl;
            rlutil::anykey();
            showScore = true;
        }
    }
